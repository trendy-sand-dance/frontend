...
import { DATABASE_URL, GAMESERVER_URL } from '../../config';

async function notifyGameserver(username: string, newUsername: string | null, newAvatar: string | null): Promise<void> {

  const data = JSON.stringify({ newUsername, newAvatar });

  try {

    const response = await fetch(`${GAMESERVER_URL}/game/editPlayer/${username}`, {
      method: 'PUT',
      headers: {
    ...
    console.error("Error occurred in notifyGameserver: ", error);
